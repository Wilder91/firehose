...
import EnviroItem from "./enviro-item";

function EnviroScraper({ neighborhood }) {
  const [enviroData, setEnviroData] = useState(null);
  ...
  function fetchData() {
    fetch(
      "https://services1.arcgis.com/UWYHeuuJISiGmgXx/arcgis/rest/services/ECB/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson",
      {
        method: "GET",
        headers: new Headers({}),
      }
      ...
      {searchAttempted && ( // Display the message only if a search has been attempted
        neighborhoodFound ? (
          neighborhoodData && !isCleared ? ( // Check if neighborhoodData is not null before rendering
            <EnviroItem neighborhoodData={neighborhoodData} />
          ) : null
        ) : (
          <p>
